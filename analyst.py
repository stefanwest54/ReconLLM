...
Covers the full engagement kill chain: RECON → FOOTHOLD → ESCALATION → INTRUSION.
Phase is determined by the analyst from confirmed loot and findings — no manual mode switch needed.
"""
...
def extract_loot_from_text(client: anthropic.Anthropic, model: str, text: str) -> dict:
    """
    Extract structured loot from arbitrary text using a non-streaming LLM call.
    Returns a dict compatible with Loot.merge(). Returns {} on any failure.
    """
...
class PenTestAnalyst:
    def __init__(self, model: str = "claude-sonnet-4-6", engagement_type: str = "pentest"):
