...
_BASE_SYSTEM_PROMPT = (
    ...
    "LOOT EXTRACTION (ONLY if actual secrets/files found; skip if empty):\n"
    ...
    "- DO NOT extract analysis notes as 'loot' — only extract actual secrets/files\n"
...
def _build_system_prompt(engagement_type: str = "pentest") -> str:
    """Build system prompt with engagement-specific context."""
    addendum = {
        "bugbounty": (
...
def _try_parse_json_response(raw: str) -> dict | None:
    """Try multiple parsing strategies for JSON from LLM response."""
    attempts = [
        ("as-is", raw),
...
class LLMAnalyst:
    """Conversational LLM analyst for penetration testing."""

    def __init__(self, model: str, system_prompt: str):
        """Initialize analyst with model and system prompt."""
        self.model = model
    ...
    def extract_loot_from_text(self, text: str, loot: "Loot") -> dict:
        """Extract structured loot from arbitrary text using LLM."""
    ...
    def _compress_history(self) -> None:
        """Summarize middle history when conversation grows too long."""
    ...
    def _stream(self, user_content: str) -> Generator[str, None, None]:
        """Stream LLM response with timeout handling and retry logic."""
    ...
    def _use_model(self, task: str):
        """Context manager to temporarily switch models for a specific task."""
    ...
    def _is_slow_nmap_command(self, cmd: str) -> bool:
        """Reject nmap commands that are known to hang or run forever."""
    ...
    def _is_local_only_command(self, cmd: str) -> bool:
        """Check if command is local-only (doesn't target the remote host)."""
        local_only = [
    ...
    def _is_already_run_command(self, cmd: str) -> bool:
        """Check if this command tool was already run (nmap, ping, etc)."""
    ...
    def _is_valid_command(self, cmd: str) -> bool:
        """Check if text is actually a command (not prose, markdown, or other junk)."""
        if not cmd or len(cmd) < 3:
            return False

        # Reject if it's clearly prose/narrative
        if cmd.lower().startswith(("the ", "a ", "this ", "that ", "use ", "run ", "check ", "look for ")):
            return False

        # Reject if it looks like a sentence (ends with . or ?)
        if cmd.rstrip().endswith(('.', '?', '!')) and not cmd.rstrip().endswith(('.txt', '.json', '.py')):
            return False

        # Reject if it's multiple sentences
        if '. ' in cmd or '? ' in cmd:
            return False

        # Reject if it's just markdown/formatting
        ...
        # Must contain a recognizable tool pattern (word followed by space and args, or have common flags)
        if ' -' not in cmd and ' /' not in cmd and cmd.count(' ') < 1:
            # Single word with no flags might be a command, but be conservative
    ...
    def extract_commands(self, text: str) -> list[str]:
        ...
        for line in searchable.splitlines():
            ...
            if match:
                ...
                # Strip shell operators that don't work with shlex.split (no shell=True)
                cmd = self._strip_shell_operators(cmd)

                if cmd and cmd not in self.seen_commands:
                    commands.append(cmd)
                    self.seen_commands.add(cmd)

        return commands

    def _strip_shell_operators(self, cmd: str) -> str:
        """Remove shell operators and redirects that won't work without shell=True."""
        # Remove all trailing redirects: 2>&1, 2>/dev/null, >/dev/null, &1, >file, etc.
    ...
    def extract_manual_steps(self, text: str) -> list[str]:
        """Extract manual steps from LLM response text."""
        steps = []
