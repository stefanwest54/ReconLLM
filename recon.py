...
from scanner import ScanResult, nmap_scan, ping_host, run_command, strip_shell_operators
...
def print_loot(loot: Loot, previous: Loot | None = None) -> None:
    if previous:
        new_loot = loot.new_items_since(previous)
        if new_loot.is_empty():
            return
        summary = new_loot.summary()
    else:
        if loot.is_empty():
            return
    ...
    print(f"{BOLD}{YELLOW}  NEW LOOT{RESET}")
...
def _analyst_response(
    ...
    new_domains = analyst.extract_domains(analysis)
    if new_domains:
        analyst.discovered_domains.update(new_domains)
        for domain in new_domains:
            good(f"Domain discovered: {domain}")

    suggested = _filter_interactive(analyst.extract_commands(analysis))

    suggested = [analyst.replace_ip_with_domain(cmd, target) for cmd in suggested]
...
def _run_resume(
    ...
    prompt = f"Resume from prior notes:\n\n{prior_notes}"
    domain_context = analyst.get_domain_context()
    if domain_context:
        prompt += f"\n{domain_context}\nPrefer using discovered domain names instead of IP addresses in suggested commands."
    ...
    for round_num in range(1, args.max_rounds + 1):
        ...
        for cmd in to_run:
            ...
            if tool_name in _INTERACTIVE_TOOLS:
                ...
                analyst.skipped_commands.add(cmd)
                continue
...
def _finalize(
    ...
    loot.skipped_commands = sorted(analyst.skipped_commands)
...
def main() -> None:
    ...
    analyst.skipped_commands = set(loot.skipped_commands)
