...
Wraps ping and nmap for initial scanning, then uses Claude to analyze results
and guide the full engagement kill chain interactively. The analyst shifts phases
naturally (RECON → FOOTHOLD → ESCALATION → INTRUSION) as findings are confirmed.
...
def print_scan_result(result: ScanResult) -> None:
    ...
    if result.stdout:
        ...
        if len(out) > 32:
            print(out[:32] + "... [output too long to print]")
        else:
            print(out)
    if result.stderr:
...
def _skip_watcher(stop_event: threading.Event, done_event: threading.Event) -> None:
    """Background thread: sets stop_event when user presses 's'."""
    fd = sys.stdin.fileno()
...
def read_user_input() -> str:
    """
    Read a freeform prompt from the user — question, pasted command output,
    website content, HTTP traffic, or anything relevant to the engagement.
    Reads until the user types END on its own line.
    """
...
def prompt_action(cmds: list[str], auto: bool) -> tuple[str, list[str]]:
    """
    Show the interactive menu and return (action, commands_to_run).
    Actions: 'run', 'input', 'quit'
    """
    if cmds:
...
def main() -> None:
    ...
    if resuming:
        # -------------------------------------------------------------------
        # Resume: brief the analyst on prior notes, skip scanning
        # -------------------------------------------------------------------
    ...
    for round_num in range(1, args.max_rounds + 1):
        ...
        # -- Freeform input (question, pasted output, web content, etc.) ----
        if action == "input":
